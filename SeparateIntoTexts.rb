def main()
  ...
  while(true)
    ...
    puts("---input---")
    puts("Enter string to split into texts")
    input = gets

    puts("---separated---")
    textSize = 150
    separated = separate(input, textSize)
    puts(separated)
  end # END DEV LOOP
end
...
def separate(encodedMessage, textSize)
...
def getNextKey(keys, cIndex, keySize)
  count = 0
  nextKey = ""
...
def getNextKeySize(keys, cIndex)
